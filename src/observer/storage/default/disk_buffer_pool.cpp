...
>>>>>>> 77a4c602b69072fb3560b1ae1cabaa44ec8e1067
...
RC DiskBufferPool::load_page(PageNum page_num, Frame* frame) {
  ...
  if (read(file_desc_, &(frame->page_), sizeof(Page)) != sizeof(Page)) {
    LOG_ERROR(
        "Failed to load page %s:%d, due to failed to read data:%s.", file_name_.c_str(), page_num, strerror(errno));
...
BufferPoolManager::~BufferPoolManager() {
  ...
  for (auto& iter : tmp_bps) { delete iter.second; }
}
